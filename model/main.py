...
base_embed = torch.tensor([1.0, 0.0, 0.0, 0.0], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
...
def DrawOut(model,lett,steps):
    ...
    for _ in range(steps):
        ...
        delta = out[:,-1:,:] + torch.rand_like(out[:,-1,:]) * 0.15
